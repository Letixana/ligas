...
//crear Liga
    ...
    public function resultado(EntityManagerInterface $em, int $id){
        ...
        try{            
            if($liga){
                $nombreLiga = $liga->getTemporada();
//ir a participantes recuperar los equipos de esa liga
//                $participante = $em->getRepository(Participante::class)->find(['liga' => $id]);
//               foreach($participante as $idParticipa){
//                    $idParticipa=$participante->getEquipo()->getId();
//aquí el where luego a los partidos de esos equipos, con select con where de local o visitante
//                    $partido = $em->getRepository(Partido::class)->findBy(['equipo' => $idParticipa]);  
//                    if($partido){
//
//                    }else {
 //                       # code...
 ...
 //               }
                ...
                if($partido){
                    ...
                    foreach($partido as $filaPartido){
                        ...
                        $dataList[] = array('equipo' => $idLocal, 'puntos' => $puntosLocal);
                        $dataList[] = array('equipo' => $idVisitante, 'puntos' => $puntosVisitante);
                    ...
                    foreach ($accumulatedValues as $equipo => $accumulatedValue) {
                                $sumArray[] = array('equipo' => $equipo, 'puntos' => $accumulatedValue);
                    }
    ...
    //             $partidos = $em->getRepository(Partido::class)->findBy(['liga' => $id]);
    //             if ($partidos) {
    //                 $equipoRepository = $em->getRepository(Equipo::class);
    //                 $golesRepository = $em->getRepository(Gol::class);
    
    //                 $equipoObjects = $equipoRepository->findAll(); // Cargar todos los equipos
    //                 $equipoMap = []; // Mapa de equipos por ID
    //                 foreach ($equipoObjects as $equipoObj) {
    //                     $equipoMap[$equipoObj->getId()] = $equipoObj;
    //                 }
    
    //                 $equipoPuntos = []; // Array para acumular puntos
    
    //                 foreach ($partidos as $partido) {
    //                     $idPartido = $partido->getId();
    
    //                     $idLocal = $partido->getLocal()->getId();
    //                     $idVisitante = $partido->getVisitante()->getId();
    //                     $golesLocal = $golesRepository->golesEquipo($idLocal, $idPartido);
    //                     $golesVisitante = $golesRepository->golesEquipo($idVisitante, $idPartido);
    
    //                     $puntosLocal = $golesLocal > $golesVisitante ? 2 : ($golesLocal === $golesVisitante ? 1 : 0);
    //                     $puntosVisitante = $golesVisitante > $golesLocal ? 2 : ($golesVisitante === $golesLocal ? 1 : 0);
    
    //                     $equipoPuntos[$idLocal] = ($equipoPuntos[$idLocal] ?? 0) + $puntosLocal;
    //                     $equipoPuntos[$idVisitante] = ($equipoPuntos[$idVisitante] ?? 0) + $puntosVisitante;
    //                 }
    
    //                 arsort($equipoPuntos); // Ordenar el array por puntos descendentes
    
    //                 $clasifica = 1;
    //                 foreach ($equipoPuntos as $equipoId => $puntos) {
    //                     $equipo = $equipoMap[$equipoId];
    //                     $datos[] = [
    //                         "Posición" => $clasifica++,
